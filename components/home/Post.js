...
const Post = ({ post }) => {
 const handleLike = post => {
  ...
  .update({
    ...
    ? firebase.firestore.FieldValue.arryayUnion(firebase.auth().currentUser.email)
    : firebase.firestore.FieldValue.arryayRemove(firebase.auth().currentUser.email)
  })
  .then(() => {
...
const Comments = ({post}) => (
    ...
    {post.comments.map((comment, index) => (
        <View key={index} style={{ flexDirection: 'row', marginTop: 60 }}>
            <Text style={{ color: 'white' }}>
